...
def create_app():
    ...
    # change hours to .001 to test session expires error
    app.config["JWT_ACCESS_TOKEN_EXPIRES"] = timedelta(hours=hours)
    # app.config["broker_url"] = os.environ.get('broker_url')
    # #celery doesn't like the CELERY_ prefix.
    # app.config['result_backend'] = os.environ.get('result_backend') #celery
    # doesn't like the CELERY_ prefix.
...
# https://flask-jwt-extended.readthedocs.io/en/stable/refreshing_tokens.html
    ...
    def expiredTokenCallback(jwt_header, jwt_data):
        ...
        if 'application/json' in acceptHeader:
            response.data = jsonify({"Message": "Token has expired",
                                    "Error": "token_expired"})
        ...
        # this will set up future requests to say not
        # authenticated (or redirect to login)
        response.set_cookie("access_token_cookie", "", max_age=0)
        return response

    @jwtManager.invalid_token_loader
    def invalid_token_callback(error):
        return jsonify({"Message": "Signature validation failed",
    ...
    def unauthorizedTokenCallback(error):
        ...
        response = make_response()
        if 'application/json' in acceptHeader:
            response.data = jsonify({"Message": "API call missing a token",
                                    "Error": "token_missing"})
            response.status_code = 401
            response.content_type = 'application/json'
    ...
    def tokenIsRevokedCallback(jwt_header, jwt_data):
        ...
        token = db.session.query(NoNoTokens).filter(NoNoTokens.jti
                                                    == jti).scalar()
    ...
    def tokenWasRevoked(jwt_header, jwt_data):
        return jsonify({"Message": "The supplied token was revoked already",
