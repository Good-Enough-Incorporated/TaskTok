...
def validate_email(form, field):
    """
    Validates that the input in the 'email' field matches the standard email
    format (user@domain.tld). Raises a ValidationError if the format is not matched.
...
def validate_password(form, field):
    """
    Validates the password field according to NIST guidelines. Ensures that the
    password is between 12 and 64 characters and contains only valid characters.
...
def validate_username(form, field):
    """
    Validates the username field to ensure it is between 4 and 20 characters
    and contains only letters and numbers. Raises a ValidationError if these
...
def validate_first_name(form, field):
    """
    Validates the first name field to ensure it is between 2 and 50
    characters and contains only allowed characters (letters, hyphens, periods,
    apostrophes, and spaces). Raises a ValidationError if these conditions are not met.
...
def validate_last_name(form, field):
    """
    Validates the last name field to ensure it is between 2 and 50 characters
    and contains only allowed characters (letters, hyphens, periods, apostrophes, and spaces
    ). Raises a ValidationError if these conditions are not met.
...
def validate_timezone(form, field):
    """
    Validates the timezone field to ensure it matches one of the known
    timezones in the pytz library. Raises a ValidationError if the timezone is unknown.
...
class NewUserForm(FlaskForm):
    """
    Form for creating a new user. Includes fields for email, username,
    and password with appropriate validators for each field.
    """
...
class ResetPasswordForm(FlaskForm):
    """
    Form for resetting a user's password. Includes fields for new password
    and confirm password, with validators to ensure they match and meet
...
class UpdatePersonalInfoForm(FlaskForm):
    """
    Form for updating a user's personal information. Includes fields for username,
    email, first name, and last name with appropriate validators for each field.
    """
...
class UpdateCredentialsForm(FlaskForm):
    """
    Form for updating a user's credentials. Includes fields for current password,
    new password, and new password confirmation with appropriate validators.
    """
...
class ForgotPasswordForm(FlaskForm):
    """
    Form for initiating the password recovery process. Includes a field for the user's
    email with the appropriate validator.
    """
    email = StringField('email', validators=[InputRequired(), validate_email])

class UpdateTimeZoneForm(FlaskForm):
    """
    Form for updating a user's timezone settings. Includes fields for timezone name and a
    boolean field for daylight savings participation.
    """
